...
@section('title')
{{$item->code.' - '.$item->value}} - Coğrafi Kod Düzenleme
@endsection
@section('css')
...
@section('content')
    @component(getAdminView('common-components.breadcrumb'))
        @slot('pagetitle')Coğrafi Kod Düzenleme @endslot
        @slot('title') {{$item->code.' - '.$item->value}} - Coğrafi Kod Düzenleme @endslot
    @endcomponent
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    ...
                    @endif
                    <form  action="{{route(getRouteWeb('geoCodes.update'),$item->id)}}"  method="post" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                             <div class="row d-flex">
                                <div class="mb-3 row">
                                    <label for="input_title" class="col-md-2 col-form-label">Kod</label>
                                    <div class="col-md-10">
                                        <input class="form-control" name="code" type="text" value="{{$item->code}}">
                                    </div>
                                </div>
                                 <div class="mb-3 row">
                                     <label for="input_title" class="col-md-2 col-form-label">Sıralama</label>
                                     <div class="col-md-10">
                                         <input class="form-control" name="sort" type="text" value="{{$item->sort}}">
                                     </div>
                                 </div>
                                 <div class="mb-3 row">
                                     <label for="input_title" class="col-md-2 col-form-label">Değer</label>
                                     <div class="col-md-10">
                                         <input class="form-control" name="value" type="text" value="{{$item->value}}">
                                     </div>
                                 </div>
                                 <div class="mb-3 row">
                                     <label for="input_title" class="col-md-2 col-form-label">Ülke</label>
                                     <div class="col-md-10">
                                         <input class="form-control" name="country" type="text" value="{{$item->country}}">
                                     </div>
                                 </div>
                            <div class="mb-3 row">
